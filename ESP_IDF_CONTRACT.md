...
- **Declared** — must be listed in the manifest `capabilities` array and consented by the user on first use (lazy / per-use prompts via `jpp_sdk_ensure_cap`):
  - **Tier 1** (prompted once, persisted to `/data/grants/<app_id>.json`): `http.request`, `https.request`, `ble.scan`, `ble.advertise`, `background.register`, `esp_now`.
  - **Tier 2** (prompted on first use every launch, never persisted): `files.full`, `network.bind`, `network.connect`, `ble.connect`, `ble.host`.

Two capabilities add a **second, resource-scoped** prompt on top of the capability prompt, because the capability alone does not identify what is being reached: `files.full` prompts per absolute path (never persisted) and `https.request` prompts per origin (`scheme://host[:port]`, persisted one-per-line to `/data/grants/<app_id>.origins` — a separate file, since the `.json` grant store is rebuilt from the fixed Tier-1 list and would drop anything else kept there).
...
| HTTP client | Broker serializes requests under the `http` lock | `http.request` gates cleartext `http_request()`; `https.request` gates TLS-verified `https_request()`. Both share the one `http` lock, so a single request is in flight at a time. |
| TCP sockets | Broker serializes socket calls under the `network` lock; lwIP callbacks live in `main/jpp_native_services.c` | `network.bind` gates the server side (`net_bind()`/`net_accept()`); `network.connect` gates the outbound client side (`net_connect()`). `net_recv()`/`net_send()`/`net_close()` accept a socket from either. 1 listener + 2 connections; binding refused while WebDAV or the LRV server runs; all sockets closed at app teardown. |
...
| `capabilities` | list[string] | Only the eleven prompted capabilities are declarable — Tier 1: `http.request`, `https.request`, `ble.scan`, `ble.advertise`, `background.register`, `esp_now`; Tier 2: `files.full`, `network.bind`, `network.connect`, `ble.connect`, `ble.host`. Anything else is `INVALID_CAPABILITY`. |
...
- The native bridge currently exports SDK version `3` (`JPP_SDK_VERSION`) on `develop`. Level `2` is the newest level in a released firmware (**v1.1**); level `3` is unreleased and still open. `sdk_min` is a floor: the loader rejects an app whose `sdk_min` exceeds the running SDK version (`SDK_TOO_OLD`). There is no upper bound — an app built for an older SDK keeps running on newer firmware (additive, backward-compatible surface). Level `2` added outbound TCP (`net_connect`, capability `network.connect`), TLS-verified HTTP (`https_request`, capability `https.request`), and the crypto primitives (`jpp_crypto_sha256` / `sha1`, `jpp_crypto_aes256_ige_encrypt` / `_decrypt`, `jpp_crypto_modexp` / `rsa_encrypt` / `dh_compute`). Level `3` made `jpp_sdk_wrap_text()` resolvable (declared since v1.0-RTM but missing from the native symbol table), gave MicroPython bindings to sixteen calls that used to be C-only (see below), and renamed the level-2 CENTER-claim API to OK (`jpp_sdk_claim_center`/`jppsdk.claim_center`/`JPP_SDK_KEY_CENTER*`/`JPP_SDK_CENTER_CLAIM_*` → `jpp_sdk_claim_ok`/`jppsdk.claim_ok`/`JPP_SDK_KEY_OK*`/`JPP_SDK_OK_CLAIM_*`). Unlike every other level-3 addition this is a rename rather than a pure addition, but the old names are kept as deprecated aliases — same `s_symtab` entry / module-dict object as their replacement — so nothing already built needs to change; new C source using an old name gets a `-Wdeprecated-declarations` warning, MicroPython does not. `sdk_min` is unaffected for code still using the old names (`claim_center` stays a level-2 requirement); only the new `claim_ok` spelling needs level 3.
- Level `2` shipped in firmware v1.1 and is closed: folding a further addition into it would leave `sdk_min: 2` meaning two different surfaces in the field. **Level `3` is unreleased and open** — further backward-compatible additions before the next release belong in level `3` rather than minting level `4`. A level is closed by a release, not by being merged.
- Available from both C and MicroPython (`jppsdk` module): `set_frame()`, `request_close()`, `log()`, `dialog()`, `list()`, `input()`, `poll_key()`, `wait_key()`, `canvas_write()`, `canvas_clear()`, `canvas_draw_pixel()`, `canvas_fullscreen()`, `buzzer_tone()`, `buzzer_play()`, `buzzer_play_sequence()`, `buzzer_play_sequence_async()`, `buzzer_stop()`, `led_set_color()`, `led_off()`, `wakelock_acquire()`, `wakelock_release()`, `device_status()`, `get_time()`, `is_dummy_mode()`, `file_read()`, `file_write()`, `file_list()`, `shared_read()`, `shared_write()`, `shared_list()`, `file_open()`, `handle_read()`, `handle_write()`, `handle_list()`, `handle_close()`, `background_register()`, `http_request()`, `https_request()`, `net_bind()`, `net_accept()`, `net_connect()`, `net_recv()`, `net_send()`, `net_close()`, `ipc_send()`, `ipc_recv()`, `kv_get()`, `kv_set()`, `kv_delete()`, `ble_scan()`, `ble_advertise_start()`, `ble_advertise_stop()`, `ble_connect()`, `ble_read_char()`, `ble_write_char()`, `ble_disconnect()`, `ble_service_register()`, `ble_service_unregister()`, `espnow_send()`, `espnow_recv()`, `claim_ok()`, `request_cap()`, `confirm()`, `wrap_text()`, `file_pick()`, `ble_set_connectable()`, `ble_host_set_value()`, `ble_host_wait_write()`, `ble_host_clear()`, `crypto_sha256()`, `crypto_sha1()`, `crypto_aes256_ige_encrypt()`, `crypto_aes256_ige_decrypt()`, `crypto_modexp()`, `crypto_rsa_encrypt()`, `crypto_dh_compute()` — the last eight (`request_cap` through `crypto_dh_compute`) only gained their MicroPython bindings at level `3`; in C they have been callable since level `1` (or level `2` for the crypto primitives). `claim_ok()` itself is level-3-only under that name — it existed at level `2` as `claim_center()`/`jppsdk.claim_center`, which is still there too, deprecated (see above).
- C-only: the code-module surface `jpp_sdk_module_load()` / `jpp_sdk_module_run()` / `jpp_sdk_module_unload()` (native apps only — MicroPython has `import` instead). This is now the only native/MicroPython gap in the App SDK surface.
