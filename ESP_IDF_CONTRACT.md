...
| `capabilities` | list[string] | Only the ten prompted capabilities are declarable — Tier 1: `http.request`, `ble.scan`, `ble.advertise`, `background.register`, `esp_now`; Tier 2: `files.full`, `network.bind`, `network.connect`, `ble.connect`, `ble.host`. Anything else is `INVALID_CAPABILITY`. |
