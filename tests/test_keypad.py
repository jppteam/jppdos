"""OK gesture behaviour of jpp_keypad_core, exercised on the host.
...
def test_double_click_never_fires_without_detection(keypad_lib):
    ...
    kp.tap()
    second = kp.idle(60)
    # Two independent OKs, no OK_DOUBLE anywhere.
    assert kinds(first) == ["OK_SHORT"]
    assert kinds(second) == ["OK_SHORT"]
...
def test_short_click_is_deferred_until_the_window_closes(keypad_lib):
    ...
    assert kinds(during) == []
    after = kp.idle(120)
    assert kinds(after) == ["OK_SHORT"]


def test_second_click_inside_the_window_becomes_ok_double(keypad_lib):
    kp = Keypad(keypad_lib, detect_double_click=True)
    kp.tap()
    gap = kp.idle(60)
    second = kp.tap()
    tail = kp.idle(DOUBLE_CLICK_MS + 100)
    # The pair reports OK_DOUBLE and neither click leaks an OK.
    assert kinds(gap) == []
    assert kinds(second + tail) == ["OK_DOUBLE"]
...
def test_hold_always_reports_ok_long(keypad_lib, detect_double_click):
    kp = Keypad(keypad_lib, detect_double_click=detect_double_click)
    events = kp.press(900)
    assert "OK_LONG" in kinds(events)
    long_event = next(e for e in events if e[2] == "HOLD")
    assert long_event[1] == "OK"


def test_hold_reports_ok_long_exactly_once(keypad_lib):
    kp = Keypad(keypad_lib, detect_double_click=False)
    events = kp.press(900) + kp.idle(100)
    assert kinds(events).count("OK_LONG") == 1


def test_hold_does_not_report_a_short_click(keypad_lib):
    kp = Keypad(keypad_lib, detect_double_click=True)
    events = kp.press(900) + kp.idle(DOUBLE_CLICK_MS + 100)
    assert "OK_SHORT" not in kinds(events)
