#!/usr/bin/env python3
"""
Build the MTProto skeleton as a position-independent ELF32 shared library.

Reads build/compile_commands.json to obtain the exact -I flags that ESP-IDF
uses for jpp_core compilation, then recompiles the MTProto sources with those
flags plus -fPIC -shared -mno-relax, producing build/apps/mtproto/mtproto.bin.

Invoked by the 'mtproto_bin' CMake custom target inside idf.py build, or
directly:  python3 apps/mtproto/build_shared.py <build_dir> <src_dir>
"""
...
def main(build_dir: str, src_dir: str) -> int:
    ...
    sources = [
        os.path.join(src_dir, "src", "mtproto.c"),
        os.path.join(src_dir, "src", "mtproto_entry.c"),
    ]
    for s in sources:
    ...
    out_dir = os.path.join(build_dir, "apps", "mtproto")
    os.makedirs(out_dir, exist_ok=True)
    out_so = os.path.join(out_dir, "mtproto.bin")

    components = os.path.join(os.path.dirname(src_dir), "..", "components")
    own_includes = [
        f"-I{os.path.join(src_dir, 'include')}",
        f"-I{os.path.join(components, 'jpp_core', 'include')}",
        f"-I{os.path.join(components, 'jpp_crypto_core', 'include')}",
    ]
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print(f"Usage: {sys.argv[0]} <build_dir> <mtproto_src_dir>", file=sys.stderr)
        sys.exit(1)
