...
esp_err_t sdmmc_init_spi_crc(sdmmc_card_t* card)
{
    ...
    if (err != ESP_OK) {
        /* J++Device override: CMD59 (CRC_ON_OFF) is required by the SPI-mode
         * spec, but some cards used on this board do not implement it. Stock
         * ESP-IDF treats a CMD59 failure as fatal and aborts the mount; here
         * we downgrade it to a warning and continue (same approach taken by
         * Flipper Zero / Arduino SD). CRC16 of data transfers simply stays in
         * its power-on default state on such cards.
         * NOTE: this is the ONLY change in this vendored copy of the sdmmc
         * component; every other source is verbatim from ESP-IDF v5.5.1. */
        ESP_LOGW(TAG, "%s: sdmmc_send_cmd_crc_on_off returned 0x%x; "
                 "continuing without SPI CRC (card may not support CMD59)",
                 __func__, err);
        return ESP_OK;
    }
    return ESP_OK;
